...
impl RelaxedIK {
    pub fn new(path_to_setting: &str) -> Self {
        ...
        log::debug!(
            "rik fully initialized in {dur:.3?} - minimum possible cost {min_possible_cost:.3?}"
        );
        a
    }
    ...
    pub fn set_ik_params(
        ...
        for i in 0..self.vars.robot.num_chains {
            self.vars.goal_positions[i] = Vector3::from_row_slice(&pos_goals[3 * i..3 * i + 3]);
    ...
    pub fn repeat_solve_ik(&mut self, pos_goals: [f64; 3]) -> Result<SolverStatus, Error> {
        ...
        //init vars
        for i in 0..self.vars.robot.num_chains {
            self.vars.goal_positions[i] = Vector3::new(
                pos_goals[3 * i],
                pos_goals[3 * i + 1],
                pos_goals[3 * i + 2] + Z_OFFSET,
            );
        }
