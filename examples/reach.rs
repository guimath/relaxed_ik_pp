...
fn main() {
    ...
    let description: urdf_rs::Robot =
        urdf_rs::read_file(rik.config.robot_urdf_path.clone()).expect("robot URDF file not found");
    ...
    match args.mode {
        ...
        VisMode::Motion => {
            ...
            if args.full_move {
                // -> interpolate
                let mut grip_plan: Vec<Vec<f64>> =
                    openrr_planner::interpolate(&q1.clone(), args.duration, 0.01)
                        .unwrap()
                        .into_iter()
